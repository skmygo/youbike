...
- **衝刺結束（07:48）**：M0–M7 全部完成，加分梯隊 S1/S2/S4/S5 完成、S3 驗證後判定不值得做。
  37 個 commit、17 個公開端點全綠、系統無人自主運作中。詳見 `SUMMARY.md`。
  資源罩全程有效（峰值 1.88G/4G），00:45 那次事故後 production 再也沒被影響過
...
- **阻塞**：S3 天氣特徵（無 CWA API key，且推論時取不到即時值 → 上不了線；已跳過，詳待辦段）
...
- [~] **加分梯隊**（提前開工）
  ...
  - [x] **S2 預測不確定帶 ✅ 已上線**（02:56 公開網址驗收：`has_interval: true`，區間隨時距擴大）：
        8 個分位數模型（733 秒）——
        覆蓋率 h30 76.2%／h60 76.6%／h120 77.2%／h180 77.6%（目標 80%，略窄但誠實），
        平均帶寬 4.8→10.7 台隨時距擴大，上下界交叉率 0.04–0.28%；
        本機推論驗過（點預測 100% 落在區間內）；模型 29 檔 75.6MB 已上 S3；**build → 部署中**
  - [!] **S3 天氣特徵：阻塞，跳過**（依 CLAUDE.md 鐵律 12）
        原因：本機無中央氣象署 API key（opendata.cwa 回 401）。就算爬 CODiS 拿到 1–6 月歷史觀測，
        **線上推論時也拿不到即時觀測與預報** → 特徵訓練得出來卻上不了線，違反規劃書自己訂的
  ...
  - [x] **S3 替代方案：驗證後決定不做**（03:11）——下載人事行政總處官方辦公日曆表（2026 = 民國 115 年，
        data.gov.tw dataset 14718，無需 key）與現行資料驅動偵測對照：
        · 官方上半年 **11 個平日假日全部被偵測抓到**（零漏網）
        · **2026 全年沒有補班日** → 這個常見賣點今年不存在
        · 資料驅動另外多抓 7 天（1/2、2/13、2/25、5/5、5/29、6/10、6/26）＝連假前後的請假潮，
          行政上要上班但騎乘需求確實像假日
        ⇒ 換成官方行事曆等於**丟掉這 7 天的資訊**。模型要學的是需求型態不是行政定義，
        因此不重訓（省下約 2 小時、零風險），把對照結論寫進 /model 頁、簡報與 slides。
        佐證檔存 `ref_data/政府行政機關辦公日曆表_2026.csv`
- [x] **M7 完成**（07:48）：17 個公開端點全 200（0.33–0.85s）、git 乾淨零未推送、
      容器 up 5 小時 healthy、SUMMARY.md 完成並補上交付當下實況（無車 71 站 vs 模型預測 285 站）
...
- 07:23 全綠（M7 前）：9 端點 200、快照 07:00、即時槽 4.6%、watch60 空站 241（早尖峰持續升溫）、
  警示 216、4 容器 healthy；SUMMARY.md 已寫
- 06:37 全綠：快照 06:30 準點、即時槽 4.3%、**watch60 空站 86→174 翻倍**（早尖峰需求上升，
  預測先反應了，而規則型通知還停在 30 筆沒動＝預測比規則型早知道的實況演示）、4 容器 healthy
- 05:51 全綠：8 端點 200、預測基準 05:30、即時槽 3.7%、警示 163（critical 22）、通知維持 30 筆
  （清晨沒有新升級＝「不重複打擾」設計正確）、4 容器 healthy
- 05:05 全綠：8 端點 200、快照 05:00 準點、預測基準 05:00、即時槽 3.5%、通知 30 筆、4 容器 healthy
- 04:18 全綠：8 端點 200、快照 04:00（04:10 抓）、預測基準 04:00、即時槽 2.9%、通知 24 筆、4 容器 healthy
- 03:46 全綠且**系統自主運作中**：快照 03:30（03:40 抓）、forecast 排程 :35 跑過、
  即時槽 0.3%→2.6% 持續爬升（橋接自我修復如預期）、主動通知累積 17 筆真實推播、容器 healthy
...
- 2026-08-07 03:05 — **加分梯隊 S1/S2/S4/S5 全部完成上線**（S3 天氣阻塞跳過）：
  主動通知管道（已有真實推播事件）、預測不確定帶（覆蓋率 76–78%）、
  可放映簡報 `/slides.html`、demo GIF。健檢全綠：10 個公開端點 200／0.33–0.47 秒、
  容器健康、Dagster 03:00 準時抓取。下一步：S3 替代方案（官方辦公日曆表，含補班日），
  訓練到本機比較後確實更好才上線
